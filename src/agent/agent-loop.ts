...
import { compressHistory, type CompressionConfig } from "./context-compressor.js";
...
export async function runAgentLoop(
  ...
  // Compress history if it exceeds the threshold (before first LLM call)
  if (config.compression?.enabled !== false) {
    const result = await compressHistory(conversationHistory, llm, config.compression);
    if (result.compressed) {
      console.log(
        `[context-compressor] Compressed ${result.summarizedCount} messages → ` +
        `${result.finalCount} total (was ${result.originalCount})`
      );
    }
  }
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    for (const tc of response.toolCalls) {
      ...
      // Vault guard: block vault retrieval when tainted by web content
      if (taintedByWeb && toolContext.hardenVaultGuard &&
          tc.name === "vault" &&
          (tc.input.action === "get" || tc.input.action === "list")) {
        const blockedMsg = "BLOCKED by vault guard: vault retrieval is not allowed immediately after browsing web content, to prevent credential exfiltration. Use vault in a separate conversation turn.";
        onToolResult?.(tc.name, blockedMsg, true);
        toolResults.push({
          type: "tool_result",
          tool_use_id: tc.id,
          content: blockedMsg,
    ...
    // Update web taint: set if any tool in this iteration was web_browse
    taintedByWeb = response.toolCalls.some(tc => tc.name === "web_browse");
